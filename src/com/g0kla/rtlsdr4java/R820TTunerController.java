...
public class R820TTunerController extends RTL2832TunerController {
	...
	/*
	 * Shadow register is used to keep a cached (in-memory) copy of all registers,
	 * so that we don't have to read a full byte from a register in order to apply a
	 * masked value and then re-write the full byte. With the shadow register, we
	 * can apply the masked value to the cached value, and then just write the
	 * masked byte, skipping the need to read the byte first.
	 */
	private int[] mShadowRegister = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x83, 0x32, 0x75, 0xC0, 0x40, 0xD6, 0x6C, 0xF5,
			0x63, 0x75, 0x68, 0x6C, 0x83, 0x80, 0x00, 0x0F, 0x00, 0xC0, 0x30, 0x48, 0xCC, 0x60, 0x00, 0x54, 0xAE, 0x4A,
			0xC0 };

	// private JPanel mEditor;

	public R820TTunerController(Device device, DeviceDescriptor deviceDescriptor)
	...
	/*
	 * @Override public void apply( TunerConfiguration tunerConfig ) throws
	 * DeviceException { if( tunerConfig != null && tunerConfig instanceof
	 * R820TTunerConfiguration ) { R820TTunerConfiguration config =
	 * (R820TTunerConfiguration)tunerConfig;
	 * 
	 * try { SampleRate sampleRate = config.getSampleRate(); setSampleRate(
	 * sampleRate );
	 * 
	 * double correction = config.getFrequencyCorrection(); setFrequencyCorrection(
	 * correction );
	 * 
	 * R820TGain masterGain = config.getMasterGain(); setGain( masterGain, true );
	 * 
	 * if( masterGain == R820TGain.MANUAL ) { R820TMixerGain mixerGain =
	 * config.getMixerGain(); setMixerGain( mixerGain, true );
	 * 
	 * R820TLNAGain lnaGain = config.getLNAGain(); setLNAGain( lnaGain, true );
	 * 
	 * R820TVGAGain vgaGain = config.getVGAGain(); setVGAGain( vgaGain, true ); }
	 * 
	 * try { setFrequency( config.getFrequency() ); } catch( DeviceException se ) {
	 * //Do nothing, we couldn't set the frequency } } catch( UsbException e ) {
	...
	/**
	 * Sets the center frequency. Setting the frequency is a two-part process of
	 * setting the multiplexer and then setting the Oscillator (PLL).
	 */
	public void setTunedFrequency(long frequency) throws DeviceException {
		try {
			...
			throw new DeviceException("R820TTunerController - exception " + "while setting frequency [" + frequency
					+ "] - " + e.getLocalizedMessage());
		}
	}

	/**
	 * Overrides the same method from the RTL2832 tuner controller to apply settings
	 * specific to the R820T tuner.
	 */
	...
	public void initTuner(boolean controlI2C) throws UsbException {
		...
		// Not sure why this is always called,  In the C source this function returns in the tuner type is 
		// not TUNER_ANALOG_TV
		// See https://github.com/librtlsdr/librtlsdr/blob/master/src/tuner_r82xx.c
	...
	/**
	 * Partially implements the r82xx_set_tv_standard() method from librtlsdr. Sets
	 * standard to digital tv to support sdr operations only.
	 */
	private void setTVStandard(boolean controlI2C) throws UsbException {
		...
		for (int x = 0; x < 2; x++) {
			...
			if (!calibrationSuccessful(calibrationCode)) {
				System.out.println("Calibration NOT successful - code: " + calibrationCode);
			}
		}
		...
		writeR820TRegister(Register.FILTER_CALIBRATION_CODE, (byte) (calibrationCode | filt_q), controlI2C);

		/* Set BW, Filter gain & HP Corner */
		writeR820TRegister(Register.BANDWIDTH_FILTER_GAIN_HIGHPASS_FILTER_CORNER, (byte) 0x6B, controlI2C);
	...
	private void setPLL(long frequency, boolean controlI2C) throws UsbException {
		...
		/*
		 * Calculate the sigma-delta modulator fractional setting. If it's non-zero,
		 * power up the sdm and apply the fractional setting, otherwise turn it off
		 */
		...
		/*
		 * Check to see if the PLL locked with these divider, integral and sdm settings
		 */
		if (!isPLLLocked(controlI2C)) {
			...
			if (!isPLLLocked(controlI2C)) {
				throw new UsbException(
						"R820T Tuner Controller - couldn't " + "achieve PLL lock on frequency [" + frequency + "]");
			}
		}
	...
	/**
	 * Indicates if the Phase Locked Loop (PLL) oscillator is locked following a
	 * change in the tuned center frequency. Checks status register 2 to see if the
	 * PLL locked indicator bit is set.
	 */
	...
	private void initializeRegisters(boolean controlI2C) throws UsbException {
		for (int x = 5; x < mShadowRegister.length; x++) {
			writeI2CRegister(mDeviceHandle, mI2CAddress, (byte) x, (byte) mShadowRegister[x], controlI2C);
		}
	}
	...
	/**
	 * Writes the byte value to the specified register, optionally controlling the
	 * I2C repeater as needed.
	 */
...
//        		String.format( "%02X", value ) );
	...
	public int readR820TRegister(Register register, boolean controlI2C) throws UsbException {
		int value = readI2CRegister(mDeviceHandle, mI2CAddress, (byte) register.getRegister(), controlI2C);

		return value;
	}

	/**
	 * Sets master gain by applying gain component values to LNA, Mixer and VGA gain
	 * registers.
	 */
	...
	public enum R820TVGAGain {
		GAIN_0("0", 0x00), GAIN_26("26", 0x01), GAIN_52("52", 0x02), GAIN_82("82", 0x03), GAIN_124("124", 0x04),
		GAIN_159("159", 0x05), GAIN_183("183", 0x06), GAIN_196("196", 0x07), GAIN_210("210", 0x08),
		GAIN_242("242", 0x09), GAIN_278("278", 0x0A), GAIN_312("312", 0x0B), GAIN_347("347", 0x0C),
		GAIN_384("384", 0x0D), GAIN_419("419", 0x0E), GAIN_455("455", 0x0F);
	...
	public enum R820TLNAGain {
		AUTOMATIC("Automatic", 0x00), GAIN_0("0", 0x10), GAIN_9("9", 0x11), GAIN_21("21", 0x12), GAIN_61("61", 0x13),
		GAIN_99("99", 0x14), GAIN_112("112", 0x15), GAIN_143("143", 0x16), GAIN_165("165", 0x17), GAIN_191("191", 0x18),
		GAIN_222("222", 0x19), GAIN_248("248", 0x1A), GAIN_262("262", 0x1B), GAIN_281("281", 0x1C),
		GAIN_286("286", 0x1D), GAIN_321("321", 0x1E), GAIN_334("334", 0x1F);
	...
	public enum R820TMixerGain {
		AUTOMATIC("Automatic", 0x10), GAIN_0("0", 0x00), GAIN_5("5", 0x01), GAIN_15("15", 0x02), GAIN_25("25", 0x03),
		GAIN_44("44", 0x04), GAIN_53("53", 0x05), GAIN_63("63", 0x06), GAIN_88("88", 0x07), GAIN_105("105", 0x08),
		GAIN_115("115", 0x09), GAIN_123("123", 0x0A), GAIN_139("139", 0x0B), GAIN_152("152", 0x0C),
		GAIN_158("158", 0x0D), GAIN_161("161", 0x0E), GAIN_153("153", 0x0F);
	...
	public enum Register {
		LNA_GAIN(0x05, 0x1F), AIR_CABLE1_INPUT_SELECTOR(0x05, 0x60), LOOP_THROUGH(0x05, 0x80),
		CABLE2_INPUT_SELECTOR(0x06, 0x08), FILTER_GAIN(0x06, 0x30), PRE_DETECT(0x06, 0x40), MIXER_GAIN(0x07, 0x1F),
		IMAGE_REVERSE(0x07, 0x80), UNKNOWN_REGISTER_8(0x08, 0x3F), UNKNOWN_REGISTER_9(0x09, 0x3F),
		FILTER_CALIBRATION_CODE(0x0A, 0x1F), FILTER_CURRENT(0x0A, 0x60), CALIBRATION_TRIGGER(0x0B, 0x10),
		FILTER_CAPACITOR(0x0B, 0x60), BANDWIDTH_FILTER_GAIN_HIGHPASS_FILTER_CORNER(0x0B, 0xEF), XTAL_CHECK(0x0C, 0x0F),
		VGA_GAIN(0x0C, 0x9F), LNA_VTH_L(0x0D, 0x0), MIXER_VTH_L(0x0E, 0x0), CALIBRATION_CLOCK(0x0F, 0x04),
		FILTER_EXTENSION_WIDEST(0x0F, 0x80), PLL_XTAL_CAPACITOR(0x10, 0x03), UNKNOWN_REGISTER_10(0x10, 0x04),
		PLL_XTAL_CAPACITOR_AND_DRIVE(0x10, 0x0B), REFERENCE_DIVIDER_2(0x10, 0x10), CAPACITOR_SELECTOR(0x10, 0x1B),
		DIVIDER(0x10, 0xE0), CP_CUR(0x11, 0x38), SIGMA_DELTA_MODULATOR_POWER(0x12, 0x08), VCO_CURRENT(0x12, 0xE0),
		VERSION(0x13, 0x3F), PLL(0x14, 0x0), SIGMA_DELTA_MODULATOR_LSB(0x15, 0x0), SIGMA_DELTA_MODULATOR_MSB(0x16, 0x0),
		DRAIN(0x17, 0x08), DIVIDER_BUFFER_CURRENT(0x17, 0x30), RF_POLY_FILTER_CURRENT(0x19, 0x60),
		PLL_AUTOTUNE(0x1A, 0x0C), PLL_AUTOTUNE_VARIANT(0x1A, 0x08), AGC_CLOCK(0x1A, 0x30), RF_POLY_MUX(0x1A, 0xC3),
		TF_BAND(0x1B, 0x0), MIXER_TOP(0x1C, 0xF8), MIXER_TOP2(0X1C, 0x04), LNA_TOP(0x1D, 0x38), LNA_TOP2(0x1D, 0xC7),
		CHANNEL_FILTER_EXTENSION(0x1E, 0x60), LNA_DISCHARGE_CURRENT(0x1E, 0x1F), LOOP_THROUGH_ATTENUATION(0x1F, 0x80);
	...
	public enum FrequencyRange {
		...
		RANGE_650(650000000, 1766000000, 0x00, 0x40, 0x00, 0x00, 0x00), RANGE_UNK(0, 0, 0, 0, 0, 0, 0);
		...
		private FrequencyRange(long minFrequency, long maxFrequency, int openDrain, int rfMuxPloy, int tf_c,
				int xtalCap20p, int xtalCap10p) {
	 ...
	 * 
	 * Divider 0: 860.43 to 1782.03 MHz Divider 1: 428.43 to 889.23 MHz Divider 2:
	 * 212.43 to 457.23 MHz Divider 3: 104.43 to 219.63 MHz Divider 4: 50.43 to
	 * 108.03 MHz Divider 5: 23.43 to 52.23 MHz Divider 6: 9.93 to 24.33 MHz Divider
	 * 7: 3.18 to 10.38 MHz
	 */
	public enum FrequencyDivider {
		DIVIDER_0(0, 2, 864000000, 1785600000, 0x00, 28800000), DIVIDER_1(1, 4, 432000000, 892800000, 0x20, 14400000),
		DIVIDER_2(2, 8, 216000000, 460800000, 0x40, 7200000), DIVIDER_3(3, 16, 108000000, 223200000, 0x60, 3600000),
		DIVIDER_4(4, 32, 54000000, 111600000, 0x80, 1800000), DIVIDER_5(5, 64, 27000000, 55800000, 0xA0, 900000),
		DIVIDER_6(6, 128, 13500000, 27900000, 0xC0, 450000), DIVIDER_7(7, 256, 6750000, 13950000, 0xE0, 225000);
		...
		private FrequencyDivider(int dividerNumber, int mixerDivider, long minimumFrequency, long maximumFrequency,
		...
		/**
		 * Returns the correct frequency divider for the specified frequency, or returns
		 * divider #5, if the frequency is outside of the specified frequency ranges.
		 * 
		...
		public Integral getIntegral(long frequency) {
			...
			throw new IllegalArgumentException(
					"PLL frequency [" + frequency + "] is not valid for this frequency divider " + this.toString());
		}

		/**
		 * Calculates the 16-bit value of the sigma-delta modulator setting which
		 * represents the fractional portion of the requested frequency that is left
		 * over after subtracting the divider minimum frequency and the integral
		 * frequency units. That residual value is divided by the integral unit value to
		 * derive a 16-bit fractional value, returned as an integer
		 */
	...
	/**
	 * PLL Integral values. Each value represents one unit of the divided crystal
	 * frequency as follows:
	 * 
	 * Divider Value 64 I * 0.9 MHz 32 I * 1.8 MHz 16 I * 3.6 MHz 8 I * 7.2 MHz 4 I
	 * * 14.4 MHz 2 I * 28.8 MHz
	 *
	 */
	public enum Integral {
		I00(0, 0x44), I01(1, 0x84), I02(2, 0xC4), I03(3, 0x05), I04(4, 0x45), I05(5, 0x85), I06(6, 0xC5), I07(7, 0x06),
		I08(8, 0x46), I09(9, 0x86), I10(10, 0xC6), I11(11, 0x07), I12(12, 0x47), I13(13, 0x87), I14(14, 0xC7),
		I15(15, 0x08), I16(16, 0x48), I17(17, 0x88), I18(18, 0xC8), I19(19, 0x09), I20(20, 0x49), I21(21, 0x89),
		I22(22, 0xC9), I23(23, 0x0A), I24(24, 0x4A), I25(25, 0x8A), I26(26, 0xCA), I27(27, 0x0B), I28(28, 0x4B),
		I29(29, 0x8B), I30(30, 0xCB), I31(31, 0x0C);
