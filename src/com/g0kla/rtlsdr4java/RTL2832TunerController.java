	...
	public void init(SampleRate sampleRate) throws DeviceException
	{
		...
		catch( Exception e )
		{
			throw new DeviceException( "RTL2832 Tuner Controller - couldn't "
				+ "set default sample rate " + e.getMessage() );
		}
	...
	public static void claimInterface( DeviceHandle handle ) throws DeviceException
	{
		if( handle != null )
		{
			...
			if( result == 1 )
			{
				...
				if( result != LibUsb.SUCCESS )
				{
					throw new DeviceException( "ERROR: failed attempt to detach kernel driver [" + 
...
//							+ "device reset", e.getMessage() );
			...
			if( resetRequired )
			{
				...
				try
				{
					...
					throw new DeviceException( "device reset attempted, but lost device handle. Try restarting the application to use this device" );
				}
			}
...
//							+ "filled descriptor byte array", e.getMessage() );
    ...
    public void removeListener( Listener<ComplexBuffer> listener ) {
    	synchronized( mComplexBufferBroadcaster ) {
    		...
    		if (mBufferProcessor == null) return;
    		if( mBufferProcessor.isRunning() ) 
    			mBufferProcessor.stop();
    	}
    }
            	...
            	while( mRunning.get() ) {
            		...
            		for( Transfer transfer: transfers )
            		{
            			...
            			if( result == LibUsb.SUCCESS )
            			{
            				...
            				System.err.println( "ERROR: Error submitting transfer [" + 
            						LibUsb.errorName( result ) + "]" );
            			}
            		}
	    ...
	    public void run()
	    {
	        while ( !mAbort )
	        {
	            ...
	            if ( result != LibUsb.SUCCESS )
	            {
	                ...
	                System.err.println("ERROR handling usb events [" + 
	            			LibUsb.errorName( result ) + "]" );
